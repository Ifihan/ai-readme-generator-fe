...
const { cta: { containerId: CTA_CONTAINER_ID, styleId: CTA_STYLE_ID, repoContainerId: REPO_CTA_CONTAINER_ID, labels: { default: CTA_DEFAULT_LABEL, loading: CTA_LOADING_LABEL, success: CTA_SUCCESS_LABEL, error: CTA_ERROR_LABEL } }, storage: { repoListLocalKey: REPO_STORAGE_KEY }, messages: MESSAGE_DEFINITIONS } = ReadmeConstants;
const { toBackground: MESSAGE_TO_BACKGROUND, broadcast: MESSAGE_BROADCAST } = MESSAGE_DEFINITIONS;
const { getAuthStatus: MSG_GET_AUTH_STATUS, startAuth: MSG_START_AUTH, showSidePanelForRepo: MSG_SHOW_SIDE_PANEL_FOR_REPO, fetchReadmeTemplates: MSG_FETCH_README_TEMPLATES, generateReadme: MSG_GENERATE_README, saveReadme: MSG_SAVE_README, fetchBranches: MSG_FETCH_BRANCHES, createBranch: MSG_CREATE_BRANCH, getPendingRepo: MSG_GET_PENDING_REPO } = MESSAGE_TO_BACKGROUND;
const { checkAuth: MSG_CHECK_AUTH, authSuccess: MSG_AUTH_SUCCESS, authFailure: MSG_AUTH_FAILURE } = MESSAGE_BROADCAST;
let ctaButton;
let ctaMessage;
let hideTimeout;
let repoCtaContainer;
let repoCtaButton;
// --- THIS FUNCTION IS NOW FIXED ---
// It no longer accesses storage. It just sends the repo data.
async function openReadmePanel(repo) {
    try {
...
async function requestAuthStatus() {
    return new Promise((resolve) => {
        chrome.runtime.sendMessage({ type: MSG_GET_AUTH_STATUS }, (response) => {
...
async function startAuthFromContent() {
    ...
    try {
        await new Promise((resolve, reject) => {
            chrome.runtime.sendMessage({ type: MSG_START_AUTH }, (response) => {
                if (chrome.runtime.lastError) {
...
function persistRepositoriesToLocalStorage(repositories) {
...
function loadRepositoriesFromLocalStorage() {
    try {
...
function maybeShowRepositoryCta(repositories) {
...
function ensureRepoCta(repo) {
    let container = repoCtaContainer ?? document.getElementById(REPO_CTA_CONTAINER_ID);
    if (!container) {
